...
from django.http import HttpResponse, HttpResponseBadRequest, HttpResponseRedirect, Http404, FileResponse
...
class index(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        port = Portfolio.objects.order_by('-date_created')[:3]
...
def about(request, category_slug=None):
    ...
    return render(request, "MyPortfolioApp/about.html",{"file":file,"projects":projects, "category":category, "specialities":specialities, "education":education,"experience":experience, "skill":skill, "course":course, "testimonies":testimonies, "port":port })
...
def project(request):
   ...
   port_filter1 = Portfolio.objects.filter(id=1).values()
   port_filter2 = Portfolio.objects.filter(id=2).values()
   file = Files.objects.all()
